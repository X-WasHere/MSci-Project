...
describe_df(df_zprime)
describe_df(df_zprime_benchmark)
logger.info(f"plotting with {len(df_zprime)} jets")

logger.info("caclulate tagger discriminants")
discs = calc_discriminant(df_zprime, model_name, fc=0.2, ftau=0.1, split_contaminated=True) # for true bjet efficiency 
discs_combined = calc_discriminant(df_zprime, model_name, fc=0.2, ftau=0.1, split_contaminated_combined=True) # combine true & contaminated for total bjet efficiency
discs_benchmark = calc_discriminant(df_zprime_benchmark, model_name_benchmark, fc=0.2, ftau=0.1, split_simple=True)

# defining target efficiency
sig_eff = np.linspace(0.3, 1, 200)

# defining boolean arrays to select the different flavour classes
flavours = get_details(df_zprime, "HadronGhostTruthLabelID", seven_class=True)
flavours_benchmark = get_details(df_zprime_benchmark, "HadronGhostTruthLabelID", seven_class=False)
...
plot_roc.draw()
plot_roc.savefig("/home/xzcapfed/MSci/flavour_contamination/evaluation/roc/ttbar_zprime/roc-zprime-only.png", transparent=False)