...
def load_df(file_pattern, model_name, num_jets=None, batch_size=500_000):
    ...
    # ADDED: "n_tracks_ghost" so we can mask on it later
    jet_vars = [
        "pt", "eta", "eventNumber",
        "HadronGhostTruthLabelID",
        "flavour_label", "n_tracks_ghost",
...
# defining target efficiency (kept outside loop so it is the same for all plots)
...
for i, n_track in enumerate(n_tracks):
    ...
    is_light, is_c, is_b, is_tau, n_jets_light, n_jets_c, n_jets_tau = get_details(df[tracks_mask], "HadronGhostTruthLabelID")
    is_light_benchmark, is_c_benchmark, is_b_benchmark, is_tau_benchmark, n_jets_light_benchmark, n_jets_c_benchmark, n_jets_tau_benchmark = get_details(df_benchmark[tracks_mask_benchmark], "HadronGhostTruthLabelID")
    ...
    plot_roc = RocPlot(
        ...
        # Adding the track label dynamically to the plot tag
    ...
    save_path = f"/home/xzcapfed/MSci/GN3_calo/plots/roc/n_tracks/roc-zprime-ntracks-{file_suffix}.png"
