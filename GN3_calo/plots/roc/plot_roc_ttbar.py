...
# # --------- TAU TEST -----------
# fname='/home/xzcapfed/MSci/GN3_calo/logs/tauregress_calo/ckpts/epoch=014-val_loss=0.97745__test_ttbar.h5'
# fname_benchmark='/home/xzcapfed/MSci/GN3_calo/logs/tauregress_benchmark/ckpts/epoch=012-val_loss=0.99412__test_ttbar.h5'
...
is_light, is_c, is_b, is_tau, n_jets_light, n_jets_c, n_jets_tau = get_details(df, "HadronGhostTruthLabelID")
is_light_benchmark, is_c_benchmark, is_b_benchmark, is_tau_benchmark, n_jets_light_benchmark, n_jets_c_benchmark, n_jets_tau_benchmark = get_details(df_benchmark, "HadronGhostTruthLabelID")


logger.info("Calculate rejection")
# rehection for GN3 with NO calo data - SWAP THESE AROUND NEED TO RECALCULATE THE REJECTIONS 
cjets_rej_benchmark = calculate_rejection(discs_benchmark[is_b_benchmark].values, discs_benchmark[is_c_benchmark].values, sig_eff)
taujets_rej_benchmark = calculate_rejection(discs_benchmark[is_b_benchmark].values, discs_benchmark[is_tau_benchmark].values, sig_eff)
light_jets_rej_benchmark = calculate_rejection(discs_benchmark[is_b_benchmark].values, discs_benchmark[is_light_benchmark].values, sig_eff)
# rejection for GN3 with calo data
cjets_rej = calculate_rejection(discs_calo[is_b].values, discs_calo[is_c].values, sig_eff)
taujets_rej = calculate_rejection(discs_calo[is_b].values, discs_calo[is_tau].values, sig_eff)
light_jets_rej = calculate_rejection(discs_calo[is_b].values, discs_calo[is_light].values, sig_eff)
...
plot_roc.draw()
plot_roc.savefig("/home/xzcapfed/MSci/GN3_calo/plots/roc/final/roc-ttbar-calo-b.png", transparent=False)
