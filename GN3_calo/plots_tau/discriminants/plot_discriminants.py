...
is_light, is_c, is_b, is_ud, is_s, is_g, is_tau = get_details(df, "HadronGhostTruthLabelID", "PartonTruthLabelID",)
...
if 'ttbar' in fname:
    ...
    working_point = 0.30
    atlas_second_tag="$\\sqrt{s}=13$ TeV $Z'$ events \n250 GeV < $p_T$ < 6 TeV \n$f_{c}=0.2$, $f_{b}=0.1$"
    sample_type = 'zprime'
...
for tagger, linestyle in zip(taggers, linestyles):
    ...
    plot_histo.add(
        Histogram(
            ...
            bins=np.linspace(-10, 10, 25),
            label="$tau$-jets" if tagger == "GN3V01_calo" else None,
...
plot_histo.savefig(f"/home/xzcapfed/MSci/GN3_calo/plots_tau/discriminants/final/b-c-l-{sample_type}-widerbins.png", transparent=False)