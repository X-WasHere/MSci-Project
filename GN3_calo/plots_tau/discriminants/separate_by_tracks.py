...
def calc_discriminant(df, name, fc, ftau, gn2=False, split_simple=False, split=False, tau=False):
    if gn2:
        ...
        denom = (fc * df[f'{name}_pc']) + (fb * df[f'{name}_pb']) + ((1-fc-fb) * uscore) + 1e-10
        disc = nom/denom  
...
discs_benchmark_all = calc_discriminant(df_benchmark, benchmark_name, fc=0.2, ftau=0.1, tau=True) # benchmark GN3 model
discs_calo_all = calc_discriminant(df, model_name, fc=0.2, ftau=0.1, tau=True) # new GN3 model
...
for i, n_track in enumerate(n_tracks):
    if i == (len(n_tracks)-1):
        ...
        tracks_label = f'\n{n_track} ≤ n_tracks < {n_tracks[i+1]}'

    logger.info(f"calculate tagger discriminants {i} / {len(n_tracks)-1}")
    discs_benchmark = discs_benchmark_all[tracks_mask_benchmark]
    discs_calo = discs_calo_all[tracks_mask]

    # defining boolean arrays to select the different flavour classes
    is_light, is_c, is_b, is_ud, is_s, is_g, is_tau = get_details(df[tracks_mask], "HadronGhostTruthLabelID", "PartonTruthLabelID",)
    is_light_benchmark, is_c_benchmark, is_b_benchmark, is_ud_benchmark, is_s_benchmark, is_g_benchmark, is_tau_benchmark = get_details(df_benchmark[tracks_mask_benchmark], "HadronGhostTruthLabelID", "PartonTruthLabelID",)

    taggers = ["GN3V01_calo", "GN3V01_benchmark"]
    discs = {"GN3V01_calo": discs_calo, "GN3V01_benchmark": discs_benchmark}
    taujet_discs = discs_calo[is_tau]
    ...
    wp_cut = np.percentile(taujet_discs, (1-working_point) * 100)
    ...
    plot_histo = HistogramPlot(
        ...
        ymax_ratio=[1.5],
        ymin_ratio=[0.5],
        atlas_second_tag= atlas_tag + tracks_label,
    )
    ...
    for tagger, linestyle in zip(taggers, linestyles):
        ...
        plot_histo.add(
            Histogram(
                ...
                bins=np.linspace(-10, 10, 25),
                label="$c$-jets" if tagger == "GN3V01_calo" else None,
        ...
        plot_histo.add(
            Histogram(
                ...
                bins=np.linspace(-10, 10, 25),
                label="$light$-jets" if tagger == "GN3V01_calo" else None,
        ...
        plot_histo.add(
            Histogram(
                values=discs[tagger][is_tau],
                bins=np.linspace(-10, 10, 25),
                label="$\\tau$-jets" if tagger == "GN3V01_calo" else None,
    ...
    plot_path = f"/home/xzcapfed/MSci/GN3_calo/plots_tau/discriminants/n_tracks/b-c-l-{sample_type}-{n_track}.png"
    plot_histo.savefig(plot_path, transparent=False)
    ...
    plot_path = f"/home/xzcapfed/MSci/GN3_calo/plots_tau/discriminants/n_tracks/g-s-l-{sample_type}-{n_track}.png"
    plot_histo.savefig(plot_path, transparent=False)